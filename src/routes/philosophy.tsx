  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .preservation-bg {
          position: fixed; top: 0; left: 0; right: 0; bottom: 0;
          background-image: url('https://i.postimg.cc/90DCfDsG/image-2-png.jpg');
          background-size: cover; background-position: center; background-repeat: no-repeat;
          filter: grayscale(300%) invert(100%) sepia(100%) hue-rotate(180deg) saturate(70%) contrast(1.4);
        ...
        .phil-row:hover { border-color: var(--rose); background: rgba(20, 15, 15, 0.9); transform: translateX(5px); }
        .phil-title { font-size: 1.1rem; margin: 0 0 0.4rem 0; letter-spacing: 2px; color: var(--cream); }
        .reader-pane {
        ...
        @media (max-width: 768px) {
          .phil-mainframe { padding: 2rem 1.5rem !important; }
          ...
          .phil-row { flex-direction: column; align-items: flex-start; gap: 1rem; padding: 1.5rem; }
      ...
      <main className="phil-mainframe">
        ...
        <div className="top-nav">
          <div className="sys-tag">ΕΛΛΕΙΠΗΣ ΦΙΛΟΣΟΦΙΑ</div>
        ...
        <div className="box-container">
          {!activeItem ? (
            <>
              {items.map((doc: any) => (
                <div key={doc.id} className="phil-row" onClick={() => setActiveItem(doc)}>
                  <div>
                    <h2 className="phil-title">{doc.title}</h2>
                    <span style={{ fontSize: '0.75rem', opacity: 0.5 }}>Σύνταξη: {doc.author}</span>
                  </div>
                  <div style={{ fontSize: '0.85rem', color: 'var(--rose)', letterSpacing: '1px' }}>[ ΜΕΛΕΤΗ ]</div>
                </div>
              ))}
            </>
          ) : (
